...
export const getWeather = async (name) => {
    const responseCode = await fetch(`${geoCodeUrl}q=${name}&limit=1&appid=${geoCodeKey}`);
    ...
    if (geocode.length !== 0) {
        try {
            const response = await fetch(`${baseUrl}${geocode[0].lat},${geocode[0].lon}?lang=ru&app_id=${appId}&app_key=${appKey}`);
